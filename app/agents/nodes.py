...
3. practice_loops 只能有 0-2 个，只在无限重复行为确实影响结果时使用。每个 practice_loop.duration_weeks 必须小于或等于 current_phase.estimated_duration_weeks；不确定时两者使用相同周数，绝不能让循环持续时间超过当前阶段。适合循环：语言练习、跑步训练、乐器练习、稳定作息。有限数量的交付目标必须使用一次性 Action 和 checkpoint：输入包含明确总数或单一交付物时，practice_loops 必须为 []，例如“完成 12 篇写作”“完成一个 Python 作品”“发布个人网站”；选择教材、完成摸底、整理素材、提交报告也不得创建 loop。
...
5. outcome_checkpoints 必须有 1-2 个，使用确定性证据规则：numeric 的 target_value 必须是非 null 数值；self_assessment 的 target_value 必须是 1 到 5 之间的数值，禁止使用 0-1 小数（例如 0.8）；artifact 必须使用 operator=exists。
...
8. 一次性 Action + practice_loops + outcome_checkpoints 合计最多 5 个核心承诺。3 个一次性 Action + 1 个 practice loop 时只能有 1 个 outcome checkpoint。生成 JSON 前必须先计算 commitment_count = 所有 TaskTree Action 数量 + practice_loops 数量 + outcome_checkpoints 数量，并确认 commitment_count <= 5。禁止输出 3 个 Action + 1 个 practice loop + 2 个 outcome checkpoint，因为 3 + 1 + 2 = 6；可改为 3 + 1 + 1 = 5，或 2 + 1 + 2 = 5。不得依赖 Validator 再修正。
...
3. 判断必须是阶段性建议，显示置信度与信息缺口，不能使用绝对正确、保证成功等措辞。用户明确说“了解很少”“不知道”“信息不足”“缺少数据”时，current_judgment.confidence 必须为 low，不能为 medium/high。
4. basis_type=working_assumption 时 statement 必须以“假设：”两个字和冒号开头；如果 statement 不是假设，就改用 user_context 或 known_constraint，绝不能保留未标注的 working_assumption。
...
正例：用户说“我对二手车了解很少，不知道车况”时 confidence="low"；basis 中的推测写成 {"statement":"假设：维修成本可能超出预算","basis_type":"working_assumption"}。
    ...
    if context is not None and context.schema_version == 2:
        ...
        if commitment_count > 5:
            errors.append(
                ...
                f"current count is {one_off_count} Action + "
                f"{len(context.practice_loops)} practice loop + "
                f"{len(context.outcome_checkpoints)} outcome checkpoint = "
                f"{commitment_count}. Remove only the excess commitments while "
                "preserving the original intent and execution strategy."
            )
